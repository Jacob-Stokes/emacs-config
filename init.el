  ...
  (when (and ascii-face-buffer (buffer-live-p ascii-face-buffer))
    (with-current-buffer ascii-face-buffer
      ...
      (let* ((faces (if (string= current-assistant-mode "claude") claude-faces codex-faces))
             ...
             (color (if (string= current-assistant-mode "claude") "#48dbfb" "#a29bfe"))
             (label (if (string= current-assistant-mode "claude") "Claude" "Codex"))
             (window-height (window-height (get-buffer-window ascii-face-buffer)))
             (bar-line (- window-height 3)))
        ;; Face animation
        (insert "\n\n  ")
        ...
        (goto-char (point-max))
        (insert "\n")
        (dotimes (_ (- bar-line 6))
          (insert "\n"))
        (insert (propertize "═══════════════════════════════════════\n"
                           'face `(:foreground "#555555")))
        (insert " ")
        (insert (propertize "⚡ VIBEMACS " 'face `(:foreground ,color :weight bold)))
        (insert (propertize "│ " 'face `(:foreground "#555555")))
        (insert (propertize (format-time-string "%H:%M") 'face `(:foreground "#888888")))
        (insert (propertize " │ " 'face `(:foreground "#555555")))
        (insert (propertize (format "Face %d/%d"
                                   (1+ ascii-face-index)
                                   (length faces))
